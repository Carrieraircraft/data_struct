...
void CountSort(T myarray[], int length, T maxvalue) //maxvalue:要排序的关键字最大值
{
	...
	for (int i = 0; i < maxvalue; ++i) //清0 :memset也可
	{
		pArrayCount[i] = 0;
	}
	...
	for (int i = 1; i < maxvalue; ++i)  //从小到大排序
	{
		pArrayCount[i] = pArrayCount[i] + pArrayCount[i - 1];
	}

	/*for (int i = maxvalue - 1; i > 0 --i)  //从大到小排序
	{
...
void RadixSort(T myarray[], int length)
{
	...
	std::list<T*> mylist[10]; //#include <list>

	//3，意味着分别取个位，十位，百位数字
	for (int i = 0; i < 3; ++i) //为简化代码，假设已经知道待排序的数字最大不超过3位，所以这里就直接写i<3了
	{
		//(1)分配
...
{	
	...
	//第六节 桶思想的排序
	//（1）计数排序（Count Sort）:通过计数而非比较来排序。
     //适用于：待排序记录数量多，但排序关键字的范围比较小的整数数字排序情形。
	//35，22，53，33，35，29，35，55，60，33，21，29，22，52，37，46，40，29
	//引入计数数组（桶）：ArrayCount= {0}；
     //ArrayCount[35] = 3，ArrayCount[22]=2，ArrayCount[53] = 1;ArrayCount[33]=2;ArrayCount[29]=3;
     //ArrayCount[55]=1;ArrayCount[60]=1;ArrayCount[21]=1;ArrayCount[52]=1;ArrayCount[37]=1;
	 //ArrayCount[46]=1;ArrayCount[40]=1;
	//（1.2）计数排序算法效率分析
    //时间复杂度O(n)。空间复杂度O(k)：k代表计数数组大小。
	//（1.3）计数排序算法稳定性问题以及算法空间复杂度的改变
	//1，3，5，1，0，5，4，1，2，3，4，3，2，5，4，4，4，4，2，0
    //ArrayCountS[0]=2;ArrayCountS[1]=3;ArrayCountS[2]=3;ArrayCountS[3]=3;ArrayCountS[4]=6;ArrayCountS[5]=3;
	//for (int i = 1; i < maxvalue; ++i) //maxvalue=6，代表计数数组大小
	//{
	//	ArrayCountS[i] = ArrayCountS[i] + ArrayCountS[i - 1];
	//}
	 //ArrayCountS[0]=2;ArrayCountS[1]=5;ArrayCountS[2]=8;ArrayCountS[3]=11;ArrayCountS[4]=17;ArrayCountS[5]=20;
    //改造成稳定算法后，空间复杂度O(n+k)

	//（2）基数排序（Radix Sort）
	//（2.1）基本概念、实现代码:多趟排序，每趟都有要经历 分配  和 收集
	//（2.2）基数排序算法效率分析
	//总的时间复杂度O(d(n+k))：d：d趟的分配和收集。n：待排序元素数量。k：用到k个辅助队列
	//空间复杂度O(n+k)
	//稳定算法
	//（2.3）基数排序算法的应用
    ...
    //时间复杂度：O(n+nlog(n/k))
    //空间复杂度O(n+k)
    //算法稳定性取决于同一个桶中排序所采用的排序算法是否稳定。

	//第七节 内部排序算法分类、复杂度、稳定性整理表
	//原地排序——不需要申请多余存储空间的排序。
	...
	//CountSort(arr, length, 99 + 1); //对数组元素进行计数排序，注意最大元素值+1，给计数数组留出空间
