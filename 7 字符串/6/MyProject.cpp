...
public:
	...
	int StrKMPIndex(const MySString& substr,
		//int next[], 
		int nextornextval[],
		int pos = 0)
	{
	...
	//根据给的nextval下标值，求nextval[idx]的值
	...
	{		
		...
		while (next_idx < length)
		{
			...
			{
				//qz_tail_idx = next[qz_tail_idx] - 1; //这句话是最难理解的
				qz_tail_idx = nextval[qz_tail_idx] - 1;//注意可以用这句替换上一句，其实不替换也行
...
{	
	...
	//（6）KMP模式匹配算法的改进
	//（6.1）next数组使用中暴露的问题
	//next数组可能导致 字符的多次重复无效比较。
	//主串：aaacaaaabeg，子串T：aaaab
	//（6.2）nextval数组的求解步骤
	//nextval数组被看做是优化过的next数组
	//a)nextval[0]：值固定为0，和next[0]相同。
	//b)nextval[1]：next[1]-1 = 1-1=0，因为T[0] != T[1]，所以nextval[1]保持next[1]不变=1
	//c)nextval[2]：next[2]-1 = 1-1=0，因为T[0] == T[2]，所以nextval[2]要等于nextval[0]=0；
	//d)nextval[3]：next[3]-1 = 2-1=1，因为T[1] == T[3]，所以nextval[3]要等于nextval[1]=1；
	//e)nextval[4]：next[4]-1 = 3-1=2，因为T[2] == T[4]，所以nextval[4]要等于nextval[2]=0；
	//f)nextval[5]：next[5]-1 = 4-1=3，因为T[3] != T[5]，所以nextval[5]保持next[5]=4；
	//g)nextval[6]：next[6]-1 = 2-1=1，因为T[1] != T[6]，所以nextval[6]保持next[6]=2；
	//h)nextval[7]：next[7]-1 = 2-1=1，因为T[1] == T[7]，所以nextval[7]要等于nextval[1]=1；
	//i)nextval[8]：next[8]-1 = 3-1=2，因为T[2] == T[8]，所以nextval[8]要等于nextval[2]=0；
	//j)nextval[9]：next[9]-1 = 4-1=3，因为T[3] == T[9]，所以nextval[9]要等于nextval[3]=1；
	//k)nextval[10]：next[10]-1 = 5-1=4，因为T[4] == T[10]，所以nextval[10]要等于nextval[4]=0；
	//l)nextval[11]：next[11]-1 = 6-1=5，因为T[5] == T[11]，所以nextval[11]要等于nextval[5]=4；

	//求nextval数组的规则(数组下标用i表示)：
	//a)数组nextval下标i为0的元素值固定为0.
	//b)如果nextval下标i>0时，分两种情况：
	   //b.1)如果T[i] != T[next[i]-1]，则nextval[i] = next[i]；
	   //b.2)如果T[i] == T[next[i]-1]，则nextval[i] = nextval[next[i]-1];
	//（6.3）nextval数组的工作原理
	//（6.4）nextval数组的求解实现代码
	...
	cout << "本次采用典型KMP算法求解next数组和nextval数组然后利用nextval数组进行模式串查找：---" << endl;
	...
	cout << "StrKMPIndex()结果为" << mys15master.StrKMPIndex(mys15sub, mynextarray15) << endl;
	delete[] mynextarray15; //释放资源
	delete[] mynextvalarray15; //释放资源
