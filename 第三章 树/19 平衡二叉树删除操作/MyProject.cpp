	...
	public:
		...
		void DeleteElem(AVLNode<T>*& tNode, const T& e) //注意第一个参数类型
		{
			...
			else if (ptmp->leftChild != nullptr && ptmp->rightChild != nullptr)
			{
				...
				parent = ptmp; //记录父节点
				slinkobj.Push(parent); //入栈
				//(2)找到这个要删除节点的左子树的最右下节点（也可以找这个要删除节点右子树的最左下节点），将该
				     //节点的值替换到要删除的节点上
				q = ptmp->leftChild;
				...
				//上面找到的这个节点肯定没有右子树，因为找到的是左子树的最右下节点嘛！
				if (parent != nullptr)
				{
			...
			if (parent == nullptr) //有些代码可以合并，但为了方便理解，老师没有合并，大家可以自行合并
			{
				...
				{
					//这个else条件应该一直不会成立
					assert(false); //如果这行报错，可以 #include <assert.h>。
				}
			}
			//-------------------------------------
			//处理平衡因子的改变（平衡性调整）
			while (slinkobj.Empty() != true)
			{
				if (slinkobj.Pop(parent) == true)
				{
					...
					if (parent->balfac == -1 || parent->balfac == 1)
					{
						//说明parent节点原来的平衡因子为0，也就是原来左右孩子都有，那么删除任意一个孩子，
						   //除parent节点平衡因子发生变化外，任何其他的parent的父亲节点等节点的平衡因子不会发生变化，这里
							//可以直接退出
						break;
					}
					if (parent->balfac == 0)
					...
					if (slinkobj.Empty() == true)
					{
						//本条件成立，表示本次平衡性调整调整到了整个树最上面的根节点，因为平衡性调整会使树根节点发生改变，
						   //所以这里 要更新根节点的指向
						root = parent;
					}
					else
					{
						//本次平衡性调整 并没有调整到 整个树最上面的根节点，但 因为平衡性调整会使树根节点发生改变，
							//所以老根节点的孩子指针应该指向新根节点
						AVLNode<T>* pParentPoint = nullptr;
						slinkobj.GetTop(pParentPoint); //拿到老根的父节点，一定会取得成功，因为栈不为空
