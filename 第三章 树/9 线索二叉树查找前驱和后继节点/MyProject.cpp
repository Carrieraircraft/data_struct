...
public:
		...
		//中序遍历序列线索化的二叉树 查找某个节点（假设二叉树的节点各不相同）
...
int main()
{
	...
	mythreadTree.CreateThreadInBTreeAccordIO(); //在二叉树中根据中序遍历序列创建线索
	//mythreadTree.CreateThreadInBTreeAccordPO();
	//mythreadTree.CreateThreadInBTreeAccordPOSTO();
	
	mythreadTree.inOrder_Org(); //传统中序遍历递归遍历
	cout << endl;
	...
	int val = 'B';
	_nmsp1::BinaryTreeNode<int>* p = mythreadTree.SearchElem_IO(val);
	if (p != nullptr)
	{
		cout << "找到了值为" << (char)val << "的节点" << endl;
		//顺便找一下后继和前趋节点
		_nmsp1::BinaryTreeNode<int>* nx = mythreadTree.GetNextPoint_IO(p);
		if (nx != nullptr)
			cout << "后继节点值为：" << (char)nx->data << "。" << endl;

		_nmsp1::BinaryTreeNode<int>* pr = mythreadTree.GetPriorPoint_IO(p);
		if(pr != nullptr)
