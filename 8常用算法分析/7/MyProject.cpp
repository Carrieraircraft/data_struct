...
//零钱兑换-LeetCode网站第322题
class Change
{
public :
	void Calc()
	{
		int a[] = { 2,5,7 }; //3种不同面值的硬币
		int amount = 27; //最终要凑成的金额
		int asize = sizeof(a) / sizeof(int);
		int* dp = new int[amount + 1];

		vector <list<int> > zongvector; //保存凑成各种不同面额的钱所需要的最少硬币列表
		zongvector.reserve(amount + 1);
		list<int> listmiane; //凑成某个面额的钱所需要的硬币面额列表（保证硬币个数最少）

		//设置dp初始值
		dp[0] = 0; //总金额为0，不需要硬币，所以=0;
		zongvector.push_back(listmiane); //总金额为0，不需要硬币。所以此时listmiane也是空的
		...
		for (int i = 1; i <= amount; ++i)
		{
			...
			for (int m = 0; m < asize; ++m)
			{
				...
				int tmpvalue = dp[i - a[m]];

				//有些凑不出来的面值，其对应的dp[i] = INT_MAX
				if (tmpvalue == INT_MAX)
					continue;
				...
				dp[i] = min(dp[i], tmpvalue + 1); //这里的+1代表1枚硬币
			} //end for m

			//把需要的各种面值硬币记录起来供显示
			if (lastjbmz != -1)
			{
				listmiane = zongvector[i - lastjbmz];
				listmiane.push_back(lastjbmz);
			}
			zongvector.push_back(listmiane);
		} //end for i

		//输出dp数组内容，也输出一下凑成总金额为i时所需要的额最少的硬币列表
		cout << "dp数组内容如下：" << endl;
		for (int i = 0; i <= amount; ++i)
		{
			if (dp[i] == INT_MAX)
			{
				cout << "dp[" << i << "]=" << "凑不出该金额！" << endl;
			}
			else
			{
				if (zongvector[i].size() > 0)
				{
					cout << "dp[" << i << "]=" << dp[i] << "，硬币面额为：";
					//能凑出来的，显示需要的硬币面额
					for (auto iter = zongvector[i].begin(); iter != zongvector[i].end(); ++iter)
					{
						cout << *iter << "  ";
					}
					cout << endl;
...
public:
	//递归函数
	int MyCalc_DiGui(int a[], int asize, int curridx)
	{
		auto itermap = mymap.find(curridx);

		//以下内容用于显示目的begin-------------
		static int idiguitimes = 0; //统计递归次数
		cout << "递归" << idiguitimes << "次。curridx = " << curridx << "    a[curridx]=" << a[curridx];
		idiguitimes++;
		if (curridx == asize - 1)
		{
			cout << "   本次是递归出口，返回值是：" << a[curridx];
		}

		if (curridx == asize - 2)
		{
			cout << "   本次是递归出口，返回值是：" << max(a[curridx], a[curridx + 1]);
		}
		

		if(itermap == mymap.end()) //该条件满足时会单独输出换行。
			cout << endl;
		//以上内容用于显示目的end-------------
		...
		if (curridx == asize - 1)
		{
			//当来到最后一间房时，那么直接把这个房子的现金返回去
			return a[curridx];
		}

		if (curridx == asize - 2)
		{
			//当来到倒数第2间房时，直接把这个房子的现金返回去，因为选择了这间就选择不了最后一间
			return max(a[curridx], a[curridx + 1]);
		}

		//防止重复递归，直接获取以往递归得到的数据
		if (itermap != mymap.end())
		{
			cout << "  找到了以往保存的curridx ,  a[curridx],maxHe = " << curridx << "," << a[curridx] << "," << (*itermap).second << endl;
			return (*itermap).second;
		}

		int rtnvalue =  max(
			a[curridx] + MyCalc_DiGui(a,asize,curridx + 2), //如果选择当前数字，则要跳过1个才能决定后面的是否选
			MyCalc_DiGui(a,asize,curridx + 1) //如果不选择当前数字，则可以考虑选择紧挨着后面一个数字
		);
		mymap.insert(make_pair(curridx, rtnvalue));
		return rtnvalue;
	}
...
int main()
{
	...
	//（6）零钱兑换 - LeetCode网站第322题
	//给定n种不同面额的硬币，比如 2元、5元、7元。总金额27元。要求计算可以凑成总金额的所需要的最少的硬币个数。
	//7 * 1  + 5 * 4  = 5个
	//dp[i]标识凑成总金额为i的时候所需要的最少硬币个数
    //dp[11] = dp[6] + (5)  //4枚
    //dp[11] = dp[9] + (2)   
    //dp[11] = dp[4] + (7)
	// -------------------------
    ...
    //dp[11] = (2) + （2）+ (2) + (5)  //4枚
    //------------------
    //dp[i] = min( dp[i-2] + (2)   ,  dp[i-5] + (5) ，dp[i-7] + (7)  )；
    //------------------
    //dp[0] = 0 
    //------------------ 
    //遍历顺序从前向后
	...
	HouseRobber myobj;
	int a[] = { 2,7,9,3,1 };
	int asize = sizeof(a) / sizeof(int); //求得数组元素个数
	myobj.ClearMyMap();
	int rtnvalue = myobj.MyCalc_DiGui(a, asize, 0);
	cout << "偷窃得到的最高金额是：" << rtnvalue << endl;
	return 0;
}
